    ...
    for key, landmark in landmarks.items():
        ...
        if key in ['neck']:
            ...
            if landmark[1] == 0:
                ...
                # df_list2.iloc[0].x -= 1
                """
                inclination = calcu.get_inclination([center['x'], center['y']], df_list2.iloc[0])
                cal_point = calcu.testCal(inclination, df_list2.iloc[0].x, df_list2.iloc[0].y, 1)
                df_list2.iloc[0].x = cal_point['x2']
                df_list2.iloc[0].y = cal_point['y2']
                """

                # ============= 랜드마크 1 ~ 0 기준으로
                # 기울기
                inclination = calcu.get_inclination([df_list2.iloc[1]['x'], df_list2.iloc[1]['y']], df_list2.iloc[0])
                # 예측 포인트
                cal_point = calcu.testCal(inclination, df_list2.iloc[0].x, df_list2.iloc[0].y, 1)
                ...
                df_list2.iloc[0].x = cal_point['x2']
                df_list2.iloc[0].y = cal_point['y2']
                df_list2.iloc[len(x_list)-1].x = cal_point['x2']
                df_list2.iloc[len(x_list)-1].y = cal_point['y2']

                print("############### %s , %s" % (cal_point['x2'], cal_point['y2']))


                # ============= 랜드마크 126 ~ 127 기준으로
                # 기울기
                inclination = calcu.get_inclination([df_list2.iloc[126]['x'], df_list2.iloc[126]['y']], df_list2.iloc[127])
                # 예측 포인트
                cal_point = calcu.testCal(inclination, df_list2.iloc[127].x, df_list2.iloc[127].y, 1)

                df_list2.iloc[127].x = cal_point['x2']
                df_list2.iloc[127].y = cal_point['y2']
                ...
                # df_list2.iloc[0].x -= 1
                # df_list2.iloc[landmark[0]].x -= 1
                # df_list2.iloc[landmark[1]].x -= 1

                # 곡선 그레이딩
                """
                {'name': 'armhole', 'target_index': 3, 'grading_size': 10.111927269247756, 'axis': 'x', 'calculation_index': [3, 53], 'line_type': 'arc'}
                {'name': 'armhole', 'target_index': 107, 'grading_size': 10.095485747636815, 'axis': 'x', 'calculation_index': [53, 107], 'line_type': 'arc'}
                """

                # chain_proportion = 1
                # chain_main_polyline = df_list2
                #
                # # chain_main_polyline = change_block_direction(chain_block_direction, chain_main_polyline, True)
                # chain_arc_len = calcu.get_arc_len(chain_main_polyline, [127, 159]) * chain_proportion
                # chain_arc_len = 5
                #
                # print("########################## chain_arc_len : ", chain_arc_len) # 5.393451955177782
                # # cur_grading_data = {'name': 'neck_depth', 'target_index': 127, 'grading_size': 10.111927269247756, 'axis': 'x', 'calculation_index': [127, 159], 'line_type': 'arc'}
                # cur_grading_data = {'name': 'neck_depth', 'target_index': 159, 'grading_size': chain_arc_len, 'axis': 'x', 'calculation_index': [127, 159], 'line_type': 'arc'}
                #
                # # cur_grading_data = {'name': 'neck_depth', 'target_index': 0, 'grading_size': 10.111927269247756, 'axis': 'x', 'calculation_index': [159, 0], 'line_type': 'arc'}
                # # cur_grading_data = {'name': 'neck_depth', 'target_index': 127, 'grading_size': 10.111927269247756, 'axis': 'x', 'calculation_index': [127, 0], 'line_type': 'arc'}
                # fit_arg_result = calcu.fit_arc(df_list2, landmarks,
                #                                  cur_grading_data["target_index"],
                #                                  cur_grading_data["calculation_index"],
                #                                  cur_grading_data["grading_size"],
                #                                  cur_grading_data["axis"])
                #
                # print("ENDED")
                #
                # if not isinstance(fit_arg_result, str):
                #     # tmp_main_polyline = fit_arg_result
                #     df_list2 = fit_arg_result
                # else:
                #     raise Exception(fit_arg_result)
                ...
                # cur_grading_data = {'name': 'neck_depth', 'target_index': 0, 'grading_size': -1, 'axis': 'x'}
                # df_list2 = move_landmark(df_list2, landmarks,
                #                           cur_grading_data["target_index"],
                #                           cur_grading_data["grading_size"],
                #                           cur_grading_data["axis"])
                ...
                이걸로 계속 테스트 함
                """
                cur_grading_data = {'name': 'neck_depth', 'target_index': 159, 'grading_size': -1, 'axis': 'x'}
                # df_list2 = calcu.move_landmark(df_list2, landmarks,
                #                               cur_grading_data["target_index"],
                #                               cur_grading_data["grading_size"],
                #                               cur_grading_data["axis"])

                # cur_grading_data = {'name': 'neck_depth', 'target_index': 127, 'grading_size': -1, 'axis': 'x'}
                # df_list2 = move_landmark(df_list2, landmarks,
                #                           cur_grading_data["target_index"],
                #                           cur_grading_data["grading_size"],
                #                           cur_grading_data["axis"])
            ...
            # print(df_list2.x[landmark[0]])
            # print(df_list2.y[landmark[0]])
